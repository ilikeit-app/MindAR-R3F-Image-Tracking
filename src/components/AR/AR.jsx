...
const AR = memo(function AR({
	...
	const resize = useCallback(() => {
		...
		const videoRatio =
			webcam.current.video.videoWidth / webcam.current.video.videoHeight;
		...
		if (inputRatio > containerRatio) {
			...
			videoDisplayHeight =
				(videoDisplayWidth / controller.inputWidth) *
		...
		webcam.current.video.style.top =
			-(vh - container.current.clientHeight) / 2 + "px";
		webcam.current.video.style.left =
			-(vw - container.current.clientWidth) / 2 + "px";
	...
	useEffect(() => {
		const loadedMetadataHandler = () => {
			console.log("📹 Ready");
			webcam.current.video.setAttribute(
				"width",
				webcam.current.video.videoWidth
			);
			webcam.current.video.setAttribute(
		...
		webcam.current.video.addEventListener(
			"loadedmetadata",
		...
		return () => {
			webcam.current.video.removeEventListener(
				"loadedmetadata",
				loadedMetadataHandler
			);
	...
	const value = useMemo(
		() => ({ controller: arContext.controller }),
		[arContext]
	);
